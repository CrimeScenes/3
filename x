...
local boxSize = Vector2.new(100, 150)  -- Default box size, can adjust as needed
...
RunService.RenderStepped:Connect(function()
    if isToggled then
        if boxSync then
            ...
            if TargetPlayer and TargetPlayer.Character then
                local targetPosition = TargetPlayer.Character:FindFirstChild("HumanoidRootPart") or TargetPlayer.Character:FindFirstChild("Head")
                if targetPosition then
                    local screenPos = Camera:WorldToViewportPoint(targetPosition.Position)
                    ...
                    if not boxFrame then
                        ...
                        boxFrame.BackgroundColor3 = Color3.fromRGB(255, 0, 0)  -- Red box for visibility
